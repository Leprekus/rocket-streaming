 
 // 2. This code loads the IFrame Player API code asynchronously.
 var tag = document.createElement('script');

 tag.src = "https://www.youtube.com/iframe_api";
 var firstScriptTag = document.getElementsByTagName('script')[0];
 firstScriptTag.parentNode.insertBefore(tag, firstScriptTag);

 // 3. This function creates an <iframe> (and YouTube player)
 //    after the API code downloads.
 var player;
 var videoId;

 function onYouTubeIframeAPIReady() {
   player = new YT.Player('player', {
	 height: '390',
	 width: '640',
	 videoId: '-BV4IpW5aLI',
	 playerVars: {
	   'playsinline': 1
	 },
	 events: {
	   'onReady': onPlayerReady,
	   'onStateChange': onPlayerStateChange
	 }
   });
 }

 function updateVideoId(id) {
	videoId = newVideoId;
            if (player && player.loadVideoById) {
                player.loadVideoById(videoId);
            }
 }
 // 4. The API will call this function when the video player is ready.
 function onPlayerReady(event) {
   event.target.playVideo();
 }

 // 5. The API calls this function when the player's state changes.
 //    The function indicates that when playing a video (state=1),
 //    the player should play for six seconds and then stop.
 var done = false;
 function onPlayerStateChange(event) {
   if (event.data == YT.PlayerState.PLAYING && !done) {
	 setTimeout(stopVideo, 6000);
	 done = true;
   }
 }
 function stopVideo() {
   player.stopVideo();
 }

 function setVideoId() {

 }

 const inputElement = document.createElement('input');
 inputElement.id = 'videoId';
 inputElement.type = 'text';
 inputElement.placeholder = 'Enter video ID';

 const searchButtonElement = document.createElement('button');
 searchButtonElement.id = 'searchButtonElement';
 searchButtonElement.type = 'button';
 searchButtonElement.textContent = 'Search';
 searchButtonElement.addEventListener('click', (e) => {
	const videoIdElement = document.getElementById('videoId');
            const id = videoIdElement.value; // Get the value of the input element
            updateVideoId(id)
 })

// Event listener for the input field
inputElement.addEventListener('input', (event) => {
	setVideoId(event.target.value);
});

// Append the input element to the body
document.body.appendChild(inputElement);
document.body.appendChild(searchButtonElement);
